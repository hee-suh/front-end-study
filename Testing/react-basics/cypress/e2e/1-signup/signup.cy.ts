describe('회원가입 테스트', () => {
	it('사용자는 이메일과 비밀번호를 사용해서 회원가입한다', () => {
		...
		cy.get('@passwordInput')
			.invoke('val')
			.then((passwordValue) => {
				expect((passwordValue as string).trim()).to.not.be.empty;
	...
	it('비밀번호와 비밀번호 확인값이 일치하지 않는경우 에러메세지가 나타난다', () => {
		...
		cy.get('[data-cy=confirmPasswordInput]').type('mismatch password');

		// then - 에러 메시지 확인
		cy.get('[data-cy=error-message]').should('be.visible').and('have.text', '비밀번호가 일치하지 않습니다');
	});
});
